...
We use testing here to refer to code that verifies that your analytical code is working as expected.
While code documentation specifies what you expect your code to do, testing assures that the action of the code meets these expectations.

The core concept of testing is **"Does my code do what I expect it to, given realistic inputs?"**.



Testing data - hardcoded, fake, close to the tests and readable. Expected output not generated by your function.

Testing directories should mirror the source code directories. Might want one file per function/class or one per module.

Test must describe what it is testing. Python in function/method name, R in testthat... Use docstrings or comments where needed.

Tests shouldn't depend on one another. No external state.

types - positive, negative (errors, if API). positive, negative, 0, NA.

Write test shells where you intend to write test - highlights where testing is missing, easy to come back to later.
...
The example above can be described as a 'unit test'. This means that it tests the smallest unit of our code - a function. There are three so-called layers of testing:
...
[Acceptance testing](https://en.wikipedia.org/wiki/Acceptance_testing) is often considered as an additional level, and should be considered whenever developing software for specific users.
...
You can't be sure that your code works without having run it with realistic examples. Therefore, you must test the most complex and important parts of your code.

Tests:
* can verify that users expectations are met by the code.
* let you know when you've broken the expected functionality of your code.
* can be used to report or flag poor performance (e.g. modelling).
