...
>
> `r tufte::quote_footer('--- Guido van Rossum (creator of python)')`

You can't be available and responsible for long term maintenance of every piece of code that you write. 
In the future, others will inevitably use and adapt your code.
...
The most important aspect of clean code is the naming of identifiers within your code - including variables, functions and classes.
...
The variable names make sense in the context that are used later on in the code.
This removes the need for explanatory comments, as what you are trying to do can be intepreted from the code itself.

Naming is important for distguishing between similar variables.
...
If you can't describe the overall task performed by the function in a few words, then your function might be too complex.
Consider breaking the function down into a number of smaller functions.
...
Even if another programmer takes a dislike to your use of whitespace or `mixedCase`, as long as you use that style consistently in your project they will soon get used to it.

[PEP8](https://www.python.org/dev/peps/pep-0008/) is widely used to style Python code.
The [Google style guide](https://google.github.io/styleguide/Rguide.html) is often used for R, which expands on the [tidyverse style guide](https://style.tidyverse.org/).
...
Two types of tool exist.
They automate validation and repair of code style:
...
Some can be configured to ignore or detect specific types of errors.
...
If the functionality of `get_odd` needs to be modified, it need only be done once. 
This code is clearer and more concise.
...
But anything more than this may not be beneficial.
