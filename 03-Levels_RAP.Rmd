...
RAP can be viewed as a layered process, moving from a simple script containing the code, to a fully integrated package built using R or python and backed up by automated testing. As you move through the RAP levels you add tools and best practice from data science to ensure reproducibility and minimize errors in the production process.
This assumes that you start with one or more ad hoc scripts. These are likely long scripts that contain the code to do a number of steps such as data validation or producing tables. This is the start of the RAP process.
