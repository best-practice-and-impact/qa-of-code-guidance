...
This chapter covers a range of methods for documenting code.
...
However, excessive use of code comments often leads to redundancy and can make reading your code more effortful.
...
Comments that describe exactly **what** is occurring in the code, as above, are not necessary.
They may be redundant, if [good naming practices](./core-principles.html#naming) are followed to self-document the steps that occur in your code.
...
If it is difficult to understand your code without comments, then this can be an indicator that your code is overly complex and might benefit from being broken down into smaller units.
...
def root_log(x):
    ...
    #if less than e subsequent root invalid for use, raise error
    if x < exp(1):
        raise ValueError("Input is less than e.")
    
    log_x = log(x)
    root_log_x = sqrt(log_x)
...
- references to other functions or classes that are related
