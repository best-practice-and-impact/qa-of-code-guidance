# Changelog
This changelog will be updated with changes to the Duck Book including additions, changes, deletions, and fixes.

## Next release: 2026.2
### Last updated: 
### Adds:
### Changes:
### Fixes:
### Removes:

## 2026.1
### Last updated: 8th June 2026
### Removes:
- Code quality assurance checklists whilst updates and improvements are being made

## 2026.0
### Last updated: 11th March 2026
### Adds:
- Guidance on using generative AI as a coding tool

### Changes:
- Locked dependency on Jupyter Book to below version 2.0 to prevent issues with building the book

## 2025.2
### Last updated: 23rd October 2025
### Fixes:
- Updated the R command line example in the Configuration chapter

## 2025.1
### Last updated: 8th May 2025
### Changes:
- Updated instructions and link for signing up to the Learning Hub

## 2025.0
### Last updated: 10th March 2025
### Adds:
- Guidance on SQL, mocking, end-to-end testing guidance
- Sections on what to test, structuring tests, using classes, risks in skipping tests
- Modelling-relevant testing
- Additional contribution guidance

### Changes:
- Updates to parameterisation and test structuring
- Book reviewed for consistency in tone

### Fixes:
- Typos and grammar
- Broken links
- Header anchors configuration

## 2024.1
### Last updated: 16th September 2024
### Fixes:
- Typos
- Broken links

This was developed from [Keep a Changelog](https://keepachangelog.com/en/1.1.0/).
